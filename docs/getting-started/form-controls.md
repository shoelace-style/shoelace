...
<sl-animation id="shake-form" name="shakeX" duration="750" iterations="1">
  <form class="validation-styles" data-inline-validation>
    ...
    <sl-input
      ...
      type="email"
      label="Email"
      help-text="We really won't spam you."
      autocomplete="off"
      required
    ></sl-input>
...
<script type="module">
  const form = document.querySelector('.validation-styles');

  form.addEventListener(
    'invalid',
    ev => {
      // Inline validation only activated if the form element
...
<style>
  .validation-styles sl-input,
  .validation-styles sl-select {
    margin-bottom: var(--sl-spacing-medium);
  }

  /* user invalid styles */
  .validation-styles sl-input[data-user-invalid]::part(base),
  .validation-styles sl-select[data-user-invalid]::part(combobox) {
    border-color: var(--sl-color-danger-600);
  }
  ...
  /* User valid styles */
  .validation-styles sl-input[data-user-valid]::part(base),
  .validation-styles sl-select[data-user-valid]::part(combobox) {
    border-color: var(--sl-color-success-600);
  }
