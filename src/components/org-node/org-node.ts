         ...
         <div class='org-tree-node-label ' part='org-tree-node-label'>
           <div  class='org-tree-node-label-inner ${this.styleClass?this.styleClass:''}' @click=${this.onNodeClick}>
             ...
             ${!isLeaf&&this.collapsable ?
              html`<span class="org-tree-node-btn ${this.expanded?'expanded':''}"  @click=${this.onToogleNode}></span>`:''}
           </div>
         </div>
         ${!isLeaf&&this.expanded?
          html`<div class='org-tree-node-children'>${this._renderChildNode()}</div>`:''}
   `;
   }
   onNodeClick(){
   ...
   private _renderChildNode(){
     ...
     if(this.nodeData.children){
       ...
       for(let i=0,j=child.length;i<j;i++){
         ...
         result.push(html`<sl-org-node class=${classMap(classObj)}
         .nodeRender=${this.nodeRender} .nodeData=${subNode} .expanded=${expanded} .collapsible=${collapsible} ></sl-org-node>`);
       }
     }
     return result;
   }
