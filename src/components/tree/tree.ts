...
//  @watchProps(['nodeRender','selectMode','filter','filterString','filterMethod','rootNodeData'])
 ...
 private renderNodeDataTemplate(data:TreeNodeData){
    ...
    return html`<sl-tree-node  selectMode=${this.selectMode} .tree=${tree} .nodeData=${data} .nodeRender=${this.real_treeNodeRender} ></sl-tree-node>`;
 }
  handSlotChange(){
  ...
  firstUpdated(map:PropertyValues){
    ...
    let eventArray=['sl-node-click','sl-node-before-open','sl-node-before-close','sl-node-before-toogle',
    'sl-node-close','sl-node-open','sl-node-toogle'];
  ...
  render() {
    ...
    return html`<div part='base' class=${classMap(baseClass)}>
         ${this.enableFilter?html`<div part='filter'><slot name='filter'><sl-input part='filter-input' .placeholder=${this.filterInputPlaceholder} @sl-input=${this.inputFilterHanlder} .value=${String(this.filterString)}></sl-input></slot></div>`:''}
  ...
  doNodeRenderChange(){
     this.real_treeNodeRender=(node:TreeNodeData)=>{
       ...
       if(this.selectMode=='check'){
         ...
         array.push(html`<sl-checkbox .nodeData=${node} @sl-change=${this.handerCheckEvent.bind(this)} class='selectCheckbox' .checked=${typeof node[this.nodeIDProperty] !='undefined'&&this.checkedKeys.includes(node[this.nodeIDProperty]) }></sl-checkbox>`);
       }else if(this.selectMode=='radio'){
           array.push(html`<sl-radio .nodeData=${node} @sl-change=${this.handerRadioEvent.bind(this)} class='selectRadio' .checked=${typeof node[this.nodeIDProperty] !='undefined'&&this.checkedKeys==node[this.nodeIDProperty] }></sl-radio>`);
       }
       array.push(result);
