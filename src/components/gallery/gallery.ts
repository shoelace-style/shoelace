...
import {  exitFullscreen, fullscreen, getCssValue, isFullscreen } from '../../utilities/common';
...
export default class SlGallery extends LitElement {
  ...
  renderNavLefAndRight(){
    return this._loadeCurentImage? html`
  ...
  private renderThumbimages(){
    const to=this.thumb_images?.map((item,index)=>{
      return html`<button part='thumb-button' @click=${()=>this.goImageByChange(index-this.currentIndex)} class='thumb-button '
 ...
 private renderImages(){
   const itemRender=(item:string,index:number)=>{
       return html`<div>
       ${!this.layImage||Math.abs(this.currentIndex-index)<=1?html`<img @load=${()=>{this._loadeCurentImage=true;this.requestUpdate()}} part="images" class="image-gallery-image" 
       .src=${item} alt="" srcset=""  title="" />`:nothing}
      ${index==this.currentIndex&&this.imageRender&&this.image_datas?
      html`<div part='image-exta'>${this.imageRender?.call(this,this.image_datas[index],index)}</div>`:nothing}</div>`;
   }
  const to=this.images?.map((item,index)=>{
    return html`<div aria-label="Go to Slide ${index}" tabindex="-1" 
      style='transform: translate3d(${100*(index-this.currentIndex)}%, 0px, 0px);transition: all var(--sl-image-transition-time) ease-out 0s;'
       class="image-gallery-slide" ?current-image=${this.currentIndex==index}   >
...
private renderImgeNavigations(){
  return html`<div class='imgage-navigation' part='image-naviagation'><div class='imgage-navigation-wrap' part='image-naviagation-wrap'>
    ${this.images?.map((_item,index)=>{
       return html`<button ?current-image=${index==this.currentIndex} 
       part='nav-button'
        @click=${()=>this.goImageByChange(index-this.currentIndex)}></button>`
    })}
  </div></div>`;
}
private renderPauseButton(){
  ...
  return this.show_pause?
   html`<button class='nav-button button-pauseButton' @click=${()=>{this.autoPlay=!this.autoPlay;this.goImageByChange(1)}} part='pauseButon'>${!this.autoPlay? svgPause:svgPaused}</button>`:'';
}
private renderFullScreenButton(){
  ...
  return this.show_fullscreen?
   html`<button class='nav-button button-fullscreen' @click=${this.handerFullScreen} part='full-screen'>${this.isFullScreened?svgFullscreened:svgFullscreen}</button>`:'';
}
  ...
  if(thumbContainer){
    if(this.thumbPosition=='bottom'||this.thumbPosition=='top'){
       ...
       if(scroll>0 &&this.thumb_images&&this.thumb_images.length>0){
        let scrollWidth=scroll/(this.thumb_images.length-1)*(this.currentIndex);
        if(!isNaN(scrollWidth)){
      ...
      thumbContainer.style.height=getCssValue(this.renderRoot.querySelector("div.image-sliders") as HTMLElement,'height');
      let scroll=thumbContainer.scrollHeight-thumbContainer.offsetHeight;
      if(scroll>0 &&this.thumb_images&&this.thumb_images.length>0){
       let scrollHeight=scroll/(this.thumb_images.length-1)*(this.currentIndex);
...
updated(map:PropertyValues){
  ...
  render() {
    return html`<sl-resize-observer @sl-resize=${()=>this.caculateThumbPotion()}><div part='base' class=' base ${this.thumbPosition} ${this.isFullScreened?'full-screen':''}'>
         <div part='images' class='images'>
           ${this.renderImages()}
           ${this.renderNavLefAndRight()}
           ${this.renderImgeNavigations()}
