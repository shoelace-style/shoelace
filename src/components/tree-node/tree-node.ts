  ...
  private renderChildren(){
      return html`${cache(
        ...
        this.nodeData.children.map(data=>{
           return html`<sl-tree-node .nodeData=${data} .nodeRender=${this.nodeRender} .tree=${this.tree}></sl-tree-node>`
      }):'')}`
  ...
  private async _clickTrigerHander(_event:Event){
    if(this.subChildSize>0){
      ...
      if(!custEvent.defaultPrevented&&!custToogleEvent.defaultPrevented){
        ...
        await animateTo(this.childTreeNodeElement,shimKeyframesHeightAuto(isClosed ?animate_show:animate_hide,height), {
          duration: 120,
          easing: 'ease'
        });
  ...
  private renderNodeData(){
    ...
    if(this.nodeData){
      if(this.nodeData.closeable){
        result.push(html`<sl-icon @click=${this._clickTrigerHander}  part='node_toogle_icon' class='trigger-status' 
            ?empty=${this.subChildSize === 0 } .name=${this.isClose?SlTreeNode.NODE_CLOSE_ICON:SlTreeNode.NODE_OPEN_ICON} > </sl-icon>`);
      }
