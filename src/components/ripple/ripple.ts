...
import { animateCss, getCssValue, getOffset } from '../../utilities/common';
...
export default class SlRipple extends LitElement {
  ...
  firstUpdated(){
    this.addEventListener('click',  async (e:MouseEvent)=>{
        ...
        const diameter = Math.round( Math.max(
        ...
        ripple.setAttribute('style',`width:${diameter}px;height:${diameter}px;` +
