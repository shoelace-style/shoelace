...
await nextTask('Generating design tokens', () => {
  return execPromise(`node scripts/make-tokens.js --outdir "${outdir}"`, { stdio: 'inherit' });
});
...
await nextTask('Generating exported styles', () => {
  return execPromise(`node scripts/make-styles.js --outdir "${outdir}"`, { stdio: 'inherit' });
});
